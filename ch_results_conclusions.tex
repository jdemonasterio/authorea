...
As an exploratory We compared the relative sample of users
...
In \cref{ch:ch:machineLearning} we gave a practical introduction to the 

We broke the problem of long-term human migrations down to four tasks which were analyzed separately.
...
However, our findings note that the gradient boosting methods were, in general, were top performing all tasks and classifiation metrics, except in the case of the $F1$ score. 
Surprisingly enough, in some tasks the Naive Bayes 
...
\todo{Add try other ML methods such as SVM or neural nets to improve on general F1 score or compete against Naive Bayes high F1 score.}
