...
In \cref{ch:descr-risk} we presented the dataset and how it was relevant to the problem in that dynamic georeferrenced data and social interactions could be helpful to show long-term human migrations.

As a starting point we transformed the individual data into an antenna dataset which aggregated information of call patterns to and from the endemic region. This information was then presented in a visualization through heat maps. These showed georeferenced call interactions and colored cellphone antennas according to their level of vulnerable interactions, as defined previously.

These visualizations exposed a `temperature' descent from the core regions outwards, where the heat is seend concentrated in the ecoregion.
As expected, the level of vulnerable interactions per antenna gradually descends as we move further away.
We say this is expected behavior because it is consistent with findings in the literature in which calls are mostly local. %in general of a local nature.
Users in ninety percent of the cases limit antenna usage to at most four Telco antennas, for the period analyzed.

However some unexpected findings were higlighted by the risk maps.
We saw that the interactions from non-endemic antennas towards those in the endemic region are non-homogeneous.
...
In the images presented before, the differences in the vulnerable interactions were clear.
