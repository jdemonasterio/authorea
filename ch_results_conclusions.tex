...
This is because the dataset is informative enough on long-term migrations and human mobility.
...
\begin{itemize}
    ...
    \item We presented a visualization of the datasets with heat maps that showed georeferrenced call interactions and colored cellphone antennas according to their level of vulnerable interactions.
    ...
    The idea was to try to explore the human mobility patterns, as captured by the phonecall records.

    \item We broke the problem of long-term human migrations down to four tasks which were analyzed separately.
    To peform this analysis, we introduced machine learning theory and methods.
    A discussion of these methods' applications to these kinds of tasks was ellaborated.
...
In this work on CDRs, the Argentinean and Mexican case studies allow us to find that it is possible to characterize human movements of long durations.
...
The risk maps highlight that the interactions from non-endemic antennas towards those in the endemic region are non-homogenous.
...
The tetection of these antennas through the visualizations was of great value to the health Mundo Sano Foundation researchers participating in this project.
...
Then, through a number of experiments, we demonstrated how the supervised classifaction models performed across different prediction tasks.
...
In this work we took advantatge of the large user scale and temporal range of the CDRs allowed us to capture a variety of human movements throughout the country.
Migration patterns can then be used to suggest actual epidemic spread of this disease into areas not deemed endemic.




\subsection{Dawbacks on the methodology}
...
\cref{tab:all_results} shows a compendium of the results obtained from the CDR datset, using all classification models.
...
\begin{description}
    ...
    The same happens with foreign workers, which are not part of our datset.
