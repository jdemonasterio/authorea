...
This should not be surprising as we have limited the amount of available features to use, thus decreasing the necessary computer resources to compute the fit.
...
sometimes better precision
Chose the non-balanced class.

Last model had much better F1 score, comparable to random forest model and trained blazingly fast.
The information in the top 30 features of the RF models was efficiently used in the boosting. It had lots of accuracy, recall, etc.
Notable difference among hyperparameters elected. The values chosen are those in favor of a less overfit model

with tree's of a relatively high depth with 12 levels.
A similar situation happened with the number of trees grown; best configurations had more than 100 trees with the second experiment having an optimal number of 200.
...
The model's selection of best features, but only for the first two experiments. Since the last one has the same
