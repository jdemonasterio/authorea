...
\todo{Do rewording for whole gradient boosting experiment section.}

In this subsection, we explore on the task described on \cref{target4} with Gradient Boosting classifiers.
This is done in comparison with the results outlined in \label{subsection:random_forest_experiments}.
Again, we perform a cross validation procedure over the available hyperparamters of this model which are, for the most part, the same as in the Random Forests classifiers.
The
