...
By the principles given above, the algorithm will need to determine an ``optimal''wayto split a set of samples, that flow through a decision node. 
...
In both, we set a balanced weighting of positive and negative samples in the loss function. 
...
From what we can see in \cref{fig:f1_random_forest_validation_curves_depth_trees}, the cross validated error is always decreasing for increasing tree depth.
...
As explained before, the boosting method builds a high model learned from other \textit{weaker} learners where decision trees are the most commonly used base models.
...
\subsubsection{Additive Optimization of the}
...
\section{Benchmark Classifier: Naive Bayes}\label{sec:naive_bayes}
