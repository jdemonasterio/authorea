...
The full experiment ran two different tasks, where each was defined by the attributes used for our $X$ dataset.
In the first one, the procedure ran with all available features and in the second one we separated the features that had a high correlation, bigger than $0.2$, with the target variable.
For both, we recorded the outcome of the cross validation procedures for each experiment in terms of different scoring metrics, the runtime, the hyperparameters chosen and the top ten best features as given by the algorithm.
...
Minimum loss reduction  & 1 &  1 \\
Running time (s)        & 2030 &  6293 \\
Features sample at a split & 0.9 & 0.8  \\
...
\todo{Add best features comparison and comment on experiments comparison. Fall in metrics, change in hyperparams, etc..}
