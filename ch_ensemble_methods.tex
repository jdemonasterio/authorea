...
\todo{add experiment for this classifier. Talk about how fast it is to set it up, with few hyperparams and almost no prior knowledge. Talk about fast execution and runs as well.}
...
\todo{fix decision tree horizontal, whole page, h! appearence}
...
\begin{figure*}[tb]
\centering
 \makebox[\textwidth]{\includegraphics[width=.9\paperwidth]{figures/decision_tree/full_tree_map2.png}}
...
If we let the tree grow deper though, we might increase the performance of the training error as shown in \cref{figure:biasVariance}.
...
%  \centering
  ...
  \end{minipage}
\caption{ Side comparison of validation curves for two hyperparameters of the Random Forest learner. The mean CV scores for the $1 - F1$ score is shown where the nubmer of trees and the depth of the trees hyperparams were tested.}
...
From what we can see in \cref{fig:f1_random_forest_validation_curves_depth_num_trees}, the cross validated erros is always decreasing for increasing tree depth.
...
Here, we are possibly improving the generalization error, as indicated by the CV estimation.
 ...
 This might be explained by the fact that we are setting a default tree depth of 9 leves.
 A configuration of this type then might be capturing all the needed model complexity to attain
 Another plausible explanation for this is that, for this task, the forest fastly converges to the generalization error and that adding more trees doesn't certainly improve on the predictions.
...
This percentage difference signals that the max depth hyperparameter is significantly more sensible to overfitting than the number of trees parameter.
...
These results are recorded and averaged across all trees and all the split scenarios to have a score for the feature. The features with highest scores can be thought to be the most informative variables of the model.

\subsection{Experiments}\label{subsection:random_forests_experiments}

 \todo{Add experiment results for Random Forest runs and show feature importances for 1 problem.}
...
Denote all possible combinations by $\Alpha_1 \times \Alpha_2 \ldots \times  \Alpha_j$, where each hyperparameter $\alpha_i$ will take all possible predefined values in $\Alpha_i \forall i \in {1,\ldots,j}$.
In practice, we must be careful with this procedure for the Random Forest algorithm since it is costly.
The learner has $j\geq 10$ and for each $\Alpha_i$ it is normal to have more than 10 possible values.
Thus the search spans a space of more than ten thousand combinations.
Moreover, each combintation has to be cross validated on the number of folds we define, creating a very large amount of iterations during this search.
...
Also, if we were to use commodity hardware with standard tools for this procedure, the experiment could take weeks and, given our dataset size, it would not fit into computer memory.
For these reasons, we take on the task with a specialized server with a 16 core CPU and 72 gigabytes of available RAM .
The system runs a UNIX based OS and our algorithms were scripted in \cref{python3.5}.
We used the specialized machine learning package \cref{graphlab} to handle the parallelization and the distribution of system resources.
...
Also, it has an \texit{out-of-the-box} memory management for large datasets.
Wit this, we can natively handle data that is larger than the amount of memory in the server used by the server, requiring minimal user knowledge to tune.
\cref{graphlab} has allowed us to conveniently run these long experiments without risk of failure due to lack of memory and without any prior expert domain knowledge on computer systems.

For this experiment, we only considered migrant people that are non-endemic at $T_1$, but have been during $T_0.
...
\begin{itemize}\label{list:random_forest_grid_search_params}
  ...
  \item The tree's maximum depth
  \item If we pre-weighted the positive and negative classes in the loss function
  \item The maximum number of trees to grow
  \item The splitting criterion at each node
  \item The number of features available to use at each split
  \item The minimum number of splits
  \item The scoring function used to evaluate the learner
  \item The minimum loss reduction needed to allow a split

\end{itemize}


The experiment ran two different procedures, where each varied in the attributes used for our $X$ dataset. In the first one the procedure ran with all available features and in the second one we separated the features that were highly correlated ($> 0.2$) with the target variable.

\todo{add experiment results}
...
A drawback of this loss though, is that it is not robust to outliers or to noisy data. During runtime weights are constantly shifting towards misclassified samples. Then if samples are mislabeled, this will make the algorithm repetitively focus on classifying incorrectly the data.
 ...
 \todo{fix subsection of adaboost compared to RF, talk about benefits and drawbacks}
...
The Boosting models were built to classify on task \cref{target1} and were evaluted with an $F1$ score.
The cross validation procedure set 10 folds for each hyperparameter
 configuration and both experiments separately tuned on the maximum depth and the number of trees.
...
Cross validation procedures ran for
and    seconds respectively and they both excluded attributes of the user's which informed on the user's home state and its current endemic condition at $T_1$.
...
%  \centering
  ...
  \end{minipage}
\caption{ Side comparison of validation curves for two hyperparameters of the Gradient Boosting learner. The mean CV scores for the $1 - F1$ score is shown, where the nubmer of trees and the depth of the trees hyperparams were tested in seperate experiments.}
...
From what we can see in, the cross validated erros is always decreasing for increasing tree depth.
A very slight deceleration in the decrease of the cross validated error can be seen for tree depth bigger than 9.
Yet the scores keeps improving and don't worsen.
This is different to the Random Forest experiment,

where if we let the tree grow deeper, at one point we only increase the performance of the training error.
Here, we are possibly improving the generalization error, as indicated by the CV estimation.
...
At their optimal configuration, both setups reached CV score averages of at least , with a maximum of  for the forest
with trees of  levels.
The optimal number of estimators for the CV and the training sets were not the same though, were  was the best configuration for the training score whilst  was the best average for the CV set. The relative difference in these best scores was of \%.
At the same time, the relative difference of the best CV and training scores, for the maximum tree depth experiment was of \%.
This percentage difference signals that the max depth hyperparameter is significantly more sensible to overfitting than the number of trees parameter.
