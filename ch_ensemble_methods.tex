...
%  \centering
  ...
  \end{minipage}
\caption{ Side comparison of validation curves for two hyperparameters of the Random Forest learner. The mean CV scores for the $1 - F1$ score is shown where the number of trees and the depth of the trees hyperparams were tested.}
...
\cref{graphlab} has allowed us to conveniently run these long experiments without risk of failure due to lack of memory and without any prior expert domain knowledge in multi-threading or parallel computing.
...
\begin{itemize}\label{list:random_forest_grid_search_params}
  ...
  \item The fraction of features available to use at each split
...
In the first one, the procedure ran with all available features and in the second one we separated the features that had a high correlation, bigger than $0.25$, with the target variable.
This means we removed all of the features that counted the user\'s vulnerable neighbours for a given month, segmented by the direction of the call.\footnote{For a complete description on the definition of these features, please refer to \cref{tab:data_example}.}
...
Then, these feature's correlation to the target variable are indicative, upto a certain point, of the predictive power of them.

For this experimentation, we recorded the outcome of the cross validation procedures in terms of different scoring metrics, the runtime, the hyperparameters chosen and the top ten best features as given by the algorithm. The summary of these results is given in \cref{tab:random_forest_big_experiment}.
...
\label{tab:random_forest_big_experiment}
\centering
...
CV $Accuracy$ score    & 0.848 &  0.889 \\
CV $Precision$ score    & 0.788 &  0.817 \\
...
Balanced Classes        & True & True  \\
Split Criterion          & Gini &  Gini \\
Minimum loss reduction  & 10 &  1 \\
Running time (s)        & 2030 &  6293 \\
Features sample in a split (\%) & 50 & 80  \\
...
In both experiments, tt is important to note though that the elected hyperparameters are not optimal in any strict sense.
...
% \todo{add this to last concluding chapter}
% Still, the results are very relevant in answering the question of long-term migrations. High roc-auc and f1 scores for the first expriment prove that there is value in CDR data to predict long-term migrations between two regions.
...
%  \centering
  ...
  \end{minipage}
\caption{ Side comparison of validation curves for two hyperparameters of the Gradient Boosting learner. The mean CV scores for the $1 - F1$ score is shown, where the number of trees and the depth of the trees hyperparams were tested in separate experiments.}
