...
The most common methods to control the tree's depth grow a very large tree $T_0$, that will continue until it reaches a depth limit threshold that is unrestrictive. Then the tree will be pruned by removing branches and nodes to remove model complexity, with only a slight loss of accuracy.
...
Still, this is considered a poor model overall.
...
Once again the algorithm uses it as the first split in the data, even with balanced samples.
The same happens for the count of calls made from the home antenna.

The tree still does a poor predictive job at determining which users have migrated in the past.
Even though balancing classes pointed to a better predictive outcome, there are still various misrepresentations in the predicted target classes.
...
Yet this won't help our generalization error.
To tackle most of the disadvantages described by this technique, \textit{Random Forests} are what follow as a natural extension to this classifier.
...
Let ${h_k(\textbf{x}) }_{i=1}^K$\footnote{There is an abuse of notation by noting trees as $h_k(\textbf{x})$ and not $h(\textbf{x}, \Theta_k)$ } be a set of classifying trees and let $I$ denote the indicator function.
...
\begin{equation}
 ...
 \todo{do run on a problem and compare to results solely with decision trees}
