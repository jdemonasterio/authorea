...
All in all, the project involved working with more than 1 terabyte in size.
The data was used in a raw CSV format which was compressed under the Gzip\footnote{For more information on this file format, please refer to Wikipedia's article on this subject: \url{https://en.wikipedia.org/wiki/Gzip}} format.
All of the call logs under this data format were be parsed, processed, transformed and loaded to create the datasets which were structured as needed by this project.
...
We also have that parameters were tuned differently when zooming into different regions. For example: an antenna whose vulnerable percentage would be considered low at the national level can be locally high when zooming in at a more local level. These filtering parameters helped us explore these cases with more detail and provided great insight to the Mundo Sano Foundation.
...
With the data provided by the CDRs and the endemic region defined in \cref{endemic_zone_mexico}, heatmaps were generated for Mexico using the methods described in \cref{subsection:home_detection}. The first generated visualizations are depicted in \cref{fig:mapas_mexico},
...
\subsection{ Feature Correlations }\label{subsection:feature_correlations} % {Ground Truth.}
...
Where in sections further in this work (\cref{ch:ensembleMethods}), we will take into account this observation to improve on the task of predicting long-term migrations.
