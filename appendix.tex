...
\chapter{- Introduction to Machine Learning}\label[appsec]{appx:introMachineLearning}

\section{Heaviside Function}\label[appsec]{appx:sec:heaviside}

The Heaviside step function is defined as
\begin{equation}\label[appsec]{eq:heavisideDefintion}
h(z) =
\begin{cases}
&0 \ \mbox{if} \ z<0 \\
...
\section{Details of the log loss functions}\label[appsec]{appx:sec:loglossDetails}
...
\label[appsec]{figure-logLossValues}
\end{center}
\end{figure}
...
\section{Bias and variance errors in other loss functions}\label[appsec]{appx:sec:biasVarianceExtensionLoss}
...
%\begin{equation}\label[appsec]{squaredBiasDecomposition}
...
\end{equation}\label[appsec]{eq:classificationEPE}


\section{The Vapnik-Chervonenkis (VC) Dimension }\label[appsec]{appx:sec:vcDimension}
...
\end{split}
\end{equation}\label[appsec]{eq:vapnik-risk}
...
\begin{equation}\label[appsec]{vapnik-empiricalRisk}
...
\end{equation}\label[appsec]{eq:vapnik-binaryBoundProbability}
...
\subsection{VC for Prediction Error Bounds }{appx:sec:vcErrroBounds}
...
\end{equation}\label[appsec]{eq:vapnik-classificationBound}



\begin{equation}
\epsilon = a_1 \frac{h \left( \ln(\frac{a_2 n}{h} ) - \ln(\frac{\eta}{4} ) \right)}{n}
\end{equation}\label[appsec]{eq:vapnik-epsilonBound}

, and when the approximating class $F$ is finite of size $d$:

\begin{equation}
\epsilon = 2 \frac{ \ln(d) - \ln(\eta)}{n}
\end{equation}\label[appsec]{eq:vapnik-epsilonBoundSimple}
...
\end{equation}\label[appsec]{eq:vapnik-classificationBoundPrecise}
...
\section{Choice of \texorpdfstring{$K$ parameter}{Lg} }\label[appsec]{appx:sec:optimalKfoldNumber}
...
\chapter{- Ensemble Methods \& Naive Bayes Classifier}\label[appsec]{appx:ensembleBayes}

\section{Decision Tree Pruning}\label[appsec]{appx:sec:tree_pruning}
...
\end{split}
\end{equation}\label[appsec]{eq:decisionTreePruneParameters}
...
\end{equation}\label[appsec]{eq:decisionTreeCostComplexity}
...
\section{Random Forests' Predictive Error Bounds}\label[appsec]{appx:sec:rforest_predictive_error_bounds}
...
\[\label[appsec]{eq:rf-marginFunRf}
...
This characterizes the expectation taken over another function which is called the \textbf{raw margin function}\label[appsec]{eq:rf-rawMarginFun}.
...
\begin{equation}\label[appsec]{eq:rf-strength}
...
\begin{equation}\label[appsec]{eq:rf-meanCorrelation}
...
\begin{theorem}
	There exists an upper bound for the generalization error of a Random Forest which is    \begin{equation}\label[appsec]{eq:rf-PEBound}
...
\begin{proof}
	...
	\begin{equation}\label[appsec]{eq:rf-marginFunVar}
	\begin{split}
	...
	\begin{equation}\label[appsec]{eq:rf-varianceBound}
	\begin{split}
	...
	\begin{equation}\label[appsec]{eq:rf-predictiveErrorBound1}
	PE^* \leq var(mr)/s^2
	\end{equation}
	by Chebyshev's inequality.
	On the other we also have that

	\begin{equation}\label[appsec]{eq:rf-expectedVarBound}
	\begin{split}
...
\subsection{Binary Class Bounds}\label[appsec]{appx:subsec:random_forests_binary_class}
...
\section{Random Forests' Margin Function Convergence}\label[appsec]{appx:sec:rforest_margin_function_convergence}
...
\end{equation}\label[appsec]{eq:rf-PEconvergence1}

and that following the law of large numbers, there is a convergence almost everywhere of
\begin{equation}\label[appsec]{rf-PEconvergence2}
...
\section{Gradient Boosting heuristic optimization}\label[appsec]{appx:sec:boosting_optimization_heuristic}
...
\begin{equation}\label[appsec]{eq:gb-targetSteps}
\begin{split}
\hat{y}_i^0 = & 0 \\
\ldots \\
...
\begin{equation}\label[appsec]{eq:gradientBoostingTaylor}
...
\begin{equation}\label[appsec]{eq:gb-objSteps1}
...
\begin{equation}\label[appsec]{eq:gb-objSteps2}
\begin{split}
