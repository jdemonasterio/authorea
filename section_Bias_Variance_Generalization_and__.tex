...
For the next part, \textbf{x} \in $\mathrm{R}^{p}$ will denote a random input variable and \textbf{y} \in $\mathrm{R}$ will denote a random output variable with joint distribution $P\left(\textbf{x},\textbf{y}\right)$ and  $x,y$. $f$ will represent a function. 
$EPE\left(f \right) = E\left(L\left(Y - f(X) \right)^2 \right)  $ where $L(Y,f\left(X\right))$ is called the loss-function. The loss function is a non-negative real valued function chosen to penalize errors in prediction. In general 
