...
There is no correct or usual specification for the regularization term $R(f)$. As is the case with other algorithms, the regularization function is chosen through cross validation, among a list of possible forms. If we consider an $L_2$ norm approach, the model's complexity will be controlled by 
$R(f) =  \gamma T + \frac{1}{2}\lambda \sum_{j=1}^T\theta_j^2$ where $\gamma$ and $\lambda$ are hyperparameters of the model. 
