...
\]
with $\theta_j \in \mathbb{R} \ \forall j = 1,...,J$ and $ \coprod_{j=1}^J \Pi_j$ is a partition of feature space. Here $q : X \mapsto \{1,...,J\}$ is the function that maps samples to regions. In summary, $\{\theta_j, \Pi_j\}_{j=1}^J$ are the model's parameters and $J$ is a hyper-parameter.
