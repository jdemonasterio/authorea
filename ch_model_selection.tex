...
% Use Hastie tibshirani to debug this part
% train error is average prediction error (loss function) of \hat{f} over train data
%
% test or generalization errors are the EPE but only conditional to a limited set of new samples from the population distribution.
% foo
% foo
% EPE is the expected loss function for our classifier, and taking expectation over any randomness in (\hat{f}(X),Y)
% and the expectation is take  with respect to the joint distribution Pr(Y,X).
...
%	Err = \Expect_{ \textbf{X}, \textbf{Y} } \left[ L(Y,f(X))  | \mathcal{T_s} \right]
	 EPE = \Expect_{\textbf{X},\textbf{Y}} \left[ L(Y,\hat{f}(X))\right] =
	 \Expect_{\textbf{X} \left[  \Expect_{{\textbf{Y}} | \textbf{X}} \left[   L(Y,f(X))  \right]  \right]
	\end{equation}
\end{definition}
...
\end{equation}
 ...
 \cref{squaredBiasDecomposition} is hereby referred to as the \textit{bias-variance decomposition for the squared loss} where the first term is called the square of the estimator's bias.
...
% \end{equation}
% Yet this would be infeasible as well for all of our possible $f( \theta)$ models since the sampling process should be done for each specific $\theta$.
