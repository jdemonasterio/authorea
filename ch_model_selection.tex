...
\caption{ Training and cross-validated mean score along the change in max tree depth. One model is fit for each tree depth value on both the training and cross validation sets. Then their scores are compared in the seires with the CV series showing a shaded standard deviation band.
}
...
At the same time, we set to 100 the maximum descent iterations for any optimization and we balanced the classes in the loss functions, where samples of the under-represented class a higher weight in the loss function, in relation to the recprocal of their ratio towards the over-reprsented class.
...
From these results it is clear that there are no significant time differences in chosing one metric over another.
Also, all metrics favor stronger regularized modles, except for the $Accuracy$ metric which favored a very loosely regularized model in the CV procedure.
...
This is evidenced in the extremely low value for the $Precision$ scores where only a handful of positie classes can be correctly captured by the model.
...
This is because the quality of solutions vary in the way they are evaluated and this evaluation will depend on the task.
...
To further explore this, we looked at their CV mean scores for each hyperparameter setting. \cref{fig:rocauc_logreg_cv_regularized_l1} and \cref{fig:rocauc_logreg_cv_regularized_l2} show the series comparison.
