...
\todo{Finish experimentation results both for RF and GBT.}

In a second stage of experiments, we performed extensive testing both on the Random Forest and Gradient Tree Boosting learners for the problem of tagging the same users that migrated out of the endemic region, but only considering the base of users which are currently living outside of it.
...
Also, we can not say that one algorithms is better over the other because they both exceed each other across different metrics.
It seems that the Random Forest is overall better at correctly classifying positive instances, while the Booster is slightly better over the recall.

Added to this, we see that both algorithm's best fit values agree in that a large number of base learners are needed to perform better.
The same with the tree's depth, where all best-fit values were, at least, over a depth of $12$.
...
By construction, these two algorithms were the only ones that provided any information on the best-features that we can extract from the dataset, for this problem.
...
From the results we collected for these two learners, the best features can be broken down into the following categories.
...
As a final remark that we must add here, the previous best-features list incorporate some of the model's feature-target correlations, because part of these best-features were selected from the experiments run with those same features.
...
\centering
    ...
    \midrule
    Naive Bayes     & (0.84,0.82,0.75)  & (0.64,0.61,0.41)  &  (0.65,0.63,0.45)   & (0.85,0.76,0.62)   \\
...
We saw that all of the learners output a very low $Precision$ score and this impacted the overall $F1$ score.
