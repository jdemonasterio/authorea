...
When tested over with the Random Forest classifier, we saw better $F1$ scores than with the Decision Learner over a 10-fold cross-validation procedure.


As a default configuration, we rebalanced all samples in the loss function

The result of this for the

 We compared both ensemble methods results in a cross-validation procedure that established how
to the

\todo{Add that random Forests improved the decision trees overfitting for Problem 2. Gradient Boosting better in bias with a slight incrase in overfit.}
