...
We teachnically set what we meant was a vulnerable interaction and what we find is a user's home antenna.
...
These models were some of the most common teachniques found in the literature for the task we were trying to solve.
...
%\begin{table*}\centering
%%\hskip-4.0cm
%% \caption{Master resutls table comparing results for all of the classifiers run in this work. For each task and classifer, we show the its $Accuracy$, $ROC AUC$ and $F1$ test-set scores, along with the runtimes of a full cross validation procedures on the learner.}
%\ra{1.3}
%%\hskip-4.0cm
...
We still see that this model had extracted enough predictive information from the dataset to correctly classify users acording to these scores.
...
On the other end of the spectrum, we had that the Naive Bayes model had the worst $ROC AUC$ and $Accuracy$ scores for all problems where it reachead a maximum $Accuracy$ of $84\%$ for the problem of predicting users that were from the endemic region in $T_0$.
...
Overall we found that there are discrepancie in the evaluation scores across the machine learning models here analyzed.
