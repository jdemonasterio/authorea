...
We found that when making predictions on which users were endemic in the past endemic, the test score was inside a one standard deviation band of the CV score, across a varying hyperparameter value for the Logistic Regression classifier.
...
For this we used the same classifier as before but on the whole dataset in what defined our systematic approach of analyzing classifiers performance.
...
We hypothesize that this is an important setting to be used in cases where the target classes are imbalanced.
...
As we will see later in \cref{tab:master_table_results}, this task, \cref{target2}, happened to be the hardest one, as it resulted in the worst metric scores for all classifiers.
...
\todo{ Using best features from random forests in gradient boosting models incorporates some overfitting in the model since we use target information to select the best features. However, independent experiments confirm the performance of gradient tree boosting methods.}
...
\begin{description}
    ...
    \item Whole Random Forest and Gradient Boosting results from the experiments.
...
\todo{Review 4 problems and wrap the best results of each algorithm, what scorings did they have?}
...
Still, at this point it is worth to mention that the runtimes resulted from experiments that were not performed under controlled computer environments.
For example, there is no way that we can assure that all cross-validation procedures were done having the same computational resources available.
Even though results seem consistent across all problems, we know that
this results shouldn't be considered benchmark proofs.



results from runtimes were note 
...
\subsection{Low F1 Score}\label{subsec:master_table_low_f1}

\todo{ Yet with a modest performance which was first ranked on the $F1$ score for difficult problems (2, 3, 4)}

Overall, we can say that, excepting the problem of predicting users that had lived in the endemic region in the past, no model had satisfactory $F1$ scores for any of the problems. The top two $F1$ score for these three problems and for all best-fit models, were $0.62$ and $0.45$.
These scores were attained by the Naive Bayes algorithm for the problems of currently non-endemic users that were living in the endemic regionin the past, and for the problem of tagging useres tht had migrated in any direction, respectively.

Interestingly enough, these poor scores for the Naive Bayes algorithm were greatly better than the ones reached by other classifiers, when compared in relative terms. 
All of the other classifiers had extremely poor $F1$ performances in \cref{target2,target3,target4}.
The $F1$ score for all of these classifieres in these problems, was $0.194$ .

On the downside

With most scores being lower than  


models had surprisingly low $F1$ overall when best-fitting, yet high and acceptable ROC AUC.\ Models seem to overvalue recall with a huge loss in precision. 

         Other scoring metrics ended in satisfactory performing models which could accurately with maximum test scores over 81\%.


\subsection{Other Scores}\label{subsec:master_table_other_scores}

\todo{ Naive Bayes modest performance which was first ranked on the $F1$ score for difficult problems (2 and 3)}
