...
Note that there are no assumptions made here about the data which we take as given. %to us in this way.
...
%% EZECORRECTION: "first type"--> bias ()directly). "relates tot he model"--> "best theoretical capacity of minimizing the true behaviour of the target variable w.r.t. to the input." -- "However..." --> "however as bias decreases, the model complexity increases and more data is needed to train it. Models with lower bias".
...
%% EZECORRECTION :  agregaar en la ecuacion con el error de datos de entrenamiento. Con la sumatoria sobre todo el set de datos y no la esperanza. y despues la relacion teorica entre PE y el generaliztcion. 

\footnote{Note that in this definition the expectation is taken conditional on the training set and also from which the model is fit.} %% EZECORRECTION: el footnote queda colgado.The generalization error is also known as the training prediction error. The prediction error is a value very much related to the generalization error as it is its expectation:
...
%% EZECORRECTION: reordenar el parrafo de arriba swapeando las dos oraciones.
...
%% EZECORRECTION: separar la indicadora en su propio parrafo o destacar esta funcion de perdida porque es todo lo que uso en la tesis.

%% EZECORRECTION: mucho "different" y mucho "Here" --> "In our case."
...
%% EZECORRECTION: "Here the systematic values are"--->"The systematic values then become". Pasar las ecuaciones esas en su propias lineas (new line).
...
%% EZECORRECTION: Cambiar la $L$ por la perdida especifica de clasificacion.
...
%% EZECORRECTION: citar la definicion de overfitting. Y agregar : "overfits a tr set  "  its variance structure is such that the number of training samples is not enough to lower the var of this model. The result of this setting will be a learner that works very accurately on the train set but with poorly overall performance. Pero entonces todos los modelos estan overfiteados? (tomar la union de train y test?)
...
%% EZECORRECTION: Destacar esto en la grafica anterior y explicar mas.
...
In Vapnik and Chervonenkis' work, the focus is on the estimation of the prediction error through the convergence and consistency of the training error. %% EZECORRECTION: esto lo pondria antes porque es la introduccion del tema, iria en el primer parrafo.
They derive the VC dimension as an important value in determining convergence speeds for different machine learning algorithms. In this way, SLT theory provides a theoretical framework to effectively characterize these dimensions in a constructive way. By the derived analytic bounds for a number of algorithms, we are enabled to estimate the distance between the empirical error and the generalization error.
...
%% EZECORRECTION: renfatizar "represents" -- > a sample of input, output data, expressed as a random variable.
...
\footnote{For simplicity, we will assume the functions $Q$ in the class to be bounded.} %% EZECORRECTION: el footnote queda colgado.
...
The property of asymptotic convergence in the training and prediction errors is expected in any learning algorithm. 
 %% EZECORRECTION: destacar esta proxima oracion.
 In SLT, the consistency requires that the training and prediction errors' sequences not only converge to the same values, but that the sequence of minimal training errors also converge, to the minimizing value of the prediction error. At the same time, the sequence of prediction errors must converge to this same point.

 %% EZECORRECTION: destacar esta proxima oracion. Ponerlo antes y mas importante. esto explica para que hacemos esto? o porque sirve?
...
\end{equation}
 %% EZECORRECTION: no esta el epsilon aca adentro en la ecuacion.
...
\footnote{The VC dimension is briefly introduced here for the purpose of giving a theoretical approach to error estimation in machine learning methods. For a complete explanation on this topic, refer to \textcite{vapnik-nature2013}}
Note that by definition this means \textit{any} possible set shattered by $\mathcal {A}$.
 %% EZECORRECTION: el footnote esta roto y ademas  explicar un poco mas el "Note that." i.e. si existe al menos uno que pueda ser shattered.
...
where we have that $h$ is the VC dimension of the class and, when the approximating class $F$ is infinite of size $N$:
 %% EZECORRECTION: hace falta reacordarse quien es h? no lo usamos antes?
...
This technique is considered one of the most widespread to evaluate the generalization performance of a set of learners. Given a number of possible configurations or values for the tuning (hyper) parameters of an algorithm, we would want to decide which selection of these fit the best estimator, as measured by the generalization error. 
 ...
 %% EZECORRECTION: agregar:" in terms of the concepts introdueced before, the queality of f from the familiy of algorithms te mejora el epe y medirlo, is assesed with this process". CV lo haces por el EPE, no por el train error. evalur las fs de la familia de clasificadores.
...
\textbf{Tuning-parameters (hyper-parameters) Selection}:
 ...
 \begin{algorithm}%[h]
 \SetAlgoLined
 \KwResult{Data table mapping error scores $CV(\alpha) \ \forall \alpha$}
...
\subsubsection{Choice of $K$ parameter}
 ...
 %% EZECORRECTION: add K is the n umber of folds and not the size of the fold.
...
Let $L \in \mathbb R_{\ge 0}^{K \times K}$ where $(L)_{(k,k')}$ is the cost of misclassifying a class $k$ sample into $k'$. Naturally we have that $L$ is a null diagonal matrix. Then the Gini index's summands will take the reweighed form $L_{kk'} \hat{p}_{jk} \hat{p}_{jk'}$.
...
If we let $C = $ $\bigcup\limits_{m=1}^{M} C_{m}$ where each $C_m$ are zero-measured sets representing the points where the sequence is not converging. If we combine \cref{rf-PEconvergence1} and \cref{rf-PEconvergence2}, we will finally have that outside of $C$,
...
%\texttt{\section{Machine Learning in Practice: Technical Observations}\label{section-technicalObservations}
