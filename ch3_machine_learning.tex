...
If we take the conditional probability in \eqref{logit-indicatorFunction}, given the features we will have that
\begin{equation}
...
If we take into account that $P(y=1 \mid x,\theta) = 1 - P(y=0 \mid x,\theta)$ and the logistic function's derivative form (\ref{eq:logitFunction}), then the estimation of $\theta$ for $N$ samples gives the following loss function
