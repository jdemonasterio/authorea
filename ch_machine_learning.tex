...
\todo{Agregar un poco mas de redundancia en la definicion de los problemas con respecto a definiciones que ya conocemos.}

\todo{Tener una lista de 5 objetos Problems en vez de un problem con listas adentro.}
...
\todo{Avisar que vamos a retomar sobre estos problemas en los capitulos X, Y, Z, etc.}
...
In this simple case we used all of the features from $\mathrm{T}$ and we generated multiple models $f(x,C)$ where we defined
$$C = \frac{1}{\lambda}$$

Each model is built from a specific $C$ in  $[10^{-5},10^5]$  the model's negative log loss error was measured for the $\mathrm{T}$ and $\mathrm{Ts}$ sets separately. These errors are graphed in \cref{fig:log_loss_regularization_validation_curve}.

As we can see, there is a clear tendency to have beter performance across smaller values of $C$ which is equivalent to having stronger model regularizations. Note that the negative log loss will have higher values for better classifiers. Thus as a simple case and with no other manipulation of the data, we find that a regularized model helps in having a better performance across the test and train sets error.
