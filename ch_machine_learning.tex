...
Here, we set $Y$ to be our target variable which is defined for every user $u$,
where $H_u$ is the user's home antenna. The list of problems considered are defined in the following way:

Knowing which users live in the endemic region
we want to infer whether they lived in $E_Z$ in the past


\todo{correctly define all problems }
