...
\begin{problems}
	...
	\begin{enumerate}\label{def:problems}
		\item We want to infer which users lived in the endemic region in the past:
		\label{itm:target1}
		\begin{align*}
			Y_u =
			\begin{cases}
		...
		\item We want to infer which users lived in $E_Z$ in the past and then migrated:\label{itm:target2}
		\begin{align*}
			Y_u =
			\begin{cases}
		...
		\item Users that migrated to a different region: \label{itm:target3}
		\begin{align*}
			Y_u =
			\begin{cases}
		...
		\item Users that are currently not endemic and migrated. This means only analyzing those users where $H_u \in { E_{Z1} }^{\complement}$: \label{itm:target4}
		\begin{align*}
			Y_u =
			\begin{cases}
				&1 \ \mbox{if} \ {H_u \in  E_{Z0} } \\
		...
		\item \textbf{Bonus.} As an extra problem and for exemplary reasons, we considered tried to detect if a user has a high mobile footpring $M_u$, as defined in \cref{section:def_mobility_diameter}.
		Users that are currently not endemic and migrated. This means only analyzing those users where $H_u \in { E_{Z1} }^{\complement}$: \label{itm:target5}
		\begin{align*}
			Y_u =
			\begin{cases}
				&1 \ \mbox{if} \ {M_u >  1000 } \\
...
Note that not all problems can be represented by the same dataset. Problems might have different limitations on what is available as our input variable $X$. For example, in \cref{itm:target1} we have no limitations to know $H_u$ at $E_{Z1}$. This means we might as well use it as a feature for the model. On the contrary, using this same feature in \cref{itm:target1} would be a problem since we are implicitly introducing the target variable as an attribute. Thus appropriate care must be taken for each case on which attributes are available for the problem being analyzed.

With the target variables already set in \cref{def:problems}, we tackle the predictions as a supervised classification problem.


\section{A working example of a Machine Learning setup}\label{section-example}

From now on we will be talking of the training set and the test set, noted by $\mathrm{T}$  and $\mathrm{Ts}$ as our two data sources. The test set will be a part of the whole dataset that will only used at the end of the algorithmic process. This set will not take part in buidling the machine learning model and will be used only to evaluate its performance. This makes sense knowing that the objective is to build a probabilistic model which has the capacity to correctly predict the class instances of new data objects, such as those from $\mathrm{Ts}$, based on having seen information of objects from $\mathrm{T}$.
...
These questions can be shaped into the form of a taret variable. In this way, they become our machine learning problem.
...
Note however, that the optimimum parameter will be dependentof the choice of the algorithm used to construct the model. That is, we have selected the optimum $\theta$ for a Logistic Classifier with input data $X$. Other algorithms can be used for the same task, and these will have their own parameters and scores.

In our derivation the loss function rised from the assumptions proposed at the start. However, we could in fact build \textit{any} type of algorithm which given an input $x_i$ and a parametrization of the model $\theta$, outputs a value $P(y_i \mid x_i,\theta)$. And we could in fact compare the performance of that model to the one built by a Logistic Regression Classifier by using the same log loss function for both. Thus we have introduced a way of determining how to select a model from the data.
...
Regularization is widely used in problems which are ill-posed either because the model has very different performance in the training vs the test set's performance. In general, restrictions are added to the model in order to reduce the number of predictors used.
...
in practice, most optimal solutions $\theta*$ are fit from loss functions with iterative optimizations. This is done even when some models with regularization have closed-form solutions. By far the most widely practiced routin is gradient descent.
...
As an example we show here how regularization affects an instance of a Logistic Classifier over the  \cref{def:problems} \cref{itm:target1}.


\todo{add regularization figure for logloss}

%\begin{figure}[h!]
%\begin{center}
%\includegraphics[width=0.7\columnwidth]{figures/regularization/figure-figure-log_los_error_validation_curve.png}
%\caption{ Validation curve comparing log loss for different values of the $C$ regularization parameter. }
%\label{fig:log_loss_regularization_validation_curve}
%\end{center}
%\end{figure}

In this simple case we used all of the features from $\mathrm{T}$ and we generated multiple models $f(x,\cdot)$. We defined
$$C = \frac{1}{\lambda}$$
and built models for different values of $C$. Each resulting model's error was measured for the $\mathrm{T}$ and $\mathrm{Ts}$ sets seprately and graphed in \cref{fig:log_loss_regularization_validation_curve}. As we can see from this graph, there is a clear tendency to have beter performance across smaller values of $C$ which is equivalent to having stronger model regularizations. 
Then we compared the losses of each model $f(\cdot,C)$


%As a second option, we did the same as before but using a different loss function called the ``area under the receiver operating characteristic curve''. We will expand on this scoring function later in \cref{section:scoring_functions}. This function will have higher values for better classifiers and with this, the following graphic confirms the idea that having a regularized model means having better performance across training and testing sets. The tendency shows that the smaller the value of $C$, the bette the model performance.

%\begin{figure}[h!]
%\begin{center}
%\includegraphics[width=0.7\columnwidth]{figures/regularization/figure-log_loss_error_validation_curve.png}
%\caption{ Validation curve comparing ROC AUC loss for different values of the $C$ regularization parameter. }
%\label{fig:log_loss_regularization_validation_curve}
%\end{center}
%\end{figure}
