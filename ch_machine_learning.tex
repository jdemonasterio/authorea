...
\section{Long-term Human Migrations}\label{long_term}

To better characterize migrations, we decided to divide our task into four problems. All of them together give us a better overview of the relationship of CDRs to
long term migrations. These are solved using user information of how they communicated during $T_1$ and no information from $T_0$ will be used. The only difference among the problems will be in how the target variable will be defined.\footnote{In reality, certain variations of the data were considered too, were best performing features were filtered from the dataset to explore how this decreased the overall prediction ability of the algorithm.}

We process the raw data to build our target variable which will be determined as $Y$. This will always be defined for every user $u$. We take $H_u$ to be the user's home antenna. With these definitions, the list of problems considered in this work are defined in the following way:



Knowing which users live in the endemic region we want to infer whether they lived in $E_Z$ in the past:


\begin{problems}
	The following is an enumeration of the tasks we set to evaluate in this work. Together, these are different persepectives of the problem of long-term migrations predictions. 
	\begin{enumerate}\label{def:problems}
		\item We want to infer which users lived in $E_Z$ in the past:
		\label{itm:target1} 
		\begin{align*}
			Y_u = 
			\begin{cases}
...
\subsection{A working example of a Machine Learning setup}\label{section-example}
