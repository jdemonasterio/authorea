...
Then if we were to use the heavside step function as  $h(z)$  where
...
Given that $\nexists i \  s.t. \ t^i = 0$, the algorithm would then correctly classify all samples with their targets. 
...
where $H(z) \approx = \frac{1}{2} + \frac{1}{2}tanh(kz) = \frac{1}{1+e^{kz}} $ 

The logistic function has the advantages of being smooth, invertible and well defined for all real numbers whereas the image is ${0,1}$. This property helps to the  of a  . 
