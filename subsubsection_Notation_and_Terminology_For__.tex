...
In our problem, a naive approach to find the parameters would be to minimize the residual sum of squares. Typical of other scenarios such as linear regression, one would like to minimize the residual sum of squares (RSS)  


\begin{equation} \label{eq:rss}
RSS(\theta_0,..,\theta_p)  = \sum_{i=1}^n [y^i - \hat{y}^i]^2  \\
=  \sum_{i=1}^n [y^i - f( \theta \cdot x^i)]^2
