...
Regularization of models is the process in which restrictions and conditions are imposed  to the model's function $f$ through a functional $\lambda R(f)$. The regularization term is used in the optimization procedure through the loss function.
\end{definition}
...
There are a number of different functionals $R$ used to impose conditions on the used model. The most common ones include penalizing directly the $\theta$ parameter  by measuring its difference to zero. The value $\lambda$ acts here as the weight that our optimization procedure will put to the regularization part of the optimization. As an example, we could have loss function with a regularization term like the following:
...
the It also helps to build more  

is that generalization errors, , more parsimonious model is not only helps us to and 
on the number of  are put to the mode By 
