...
\begin{frame}{Objetivo Principal}
	...
	\begin{equation}%\label{expectedPredictionError}
		\begin{split}
...
\begin{frame}{ Poly fit vs. fit lineal en shattering }
...
\begin{frame}{Clasificador: ``Naive Bayes''}




	\begin{block}{Idea}
		Asumimos \textit{inocentemente} que
		$$p(X^j |X^1,\ldots,\hat{X^j},\ldots,X^p, C_k) \ = \ p(X^j | C_k) \ \forall k \in \{0,1\}$$
		% \begin{equation}
		% asdf
		% \end{equation}
	\end{block}

	\begin{block}{Funcion de perdida }

		\begin{equation}
	\hat{Y} \ = \ \mathbb{I}\left[ p(C_{1}) \  \prod_{i=1}^{n} p (X_{i}\mid C_{1})   \ >= \ \frac{1}{2} \right]
		\end{equation}
%	\underset{k\in\{1,\ldots,K\}}{\operatorname{argmax}}
	\end{block}
	% $\gamma$ es un hiperparametro para la regularizacion del modelo
\end{frame}
