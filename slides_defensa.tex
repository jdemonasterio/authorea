...
En total, cada dataset tiene mas de 10 mil millones de registros realizados por
  8 millones y 2 millones de usuarios, respectivamente para Argentina y
  México\footnote{No tenemos ninguna información personal de los usuarios como nombre, dirección, teléfono y su privacidad está asegurada mediante criptografía.}
...
\begin{frame}{Regiones focalizadas}
	...
	%A partir de estos primeros mapas se decidi\'o enfocar la visualizaci\'on y mejorar la precisi\'on en ciertas regiones fuera del Gran Chaco.
		\begin{itemize}
			...
			\item Buenos Aires
			\item Capital Federal, Sur y Norte CABA y AMBA
...
Algoritmo recursivo y aleatorio que particiona el espacio en forma binaria:

	\begin{block}{Idea}
		$$\mathcal{T}_{iz}(d,j) = \{x \in \mathcal{T} \ / \ x^j \leq d \}$$
		$$\mathcal{T}_{de}(d,j) = \mathcal{T} \setminus  \mathcal{T}_I(d,j)$$
		A partir de una función de impureza $Imp$
		\begin{align*}
	...
	\begin{block}{Función de perdida}

		\begin{align*}
				h(X_i) = \sum_{k=1}^K c_k I(X_i \in \mathcal{T}_k)
		\end{align*}
 	con $K$ el número de regiones de la partición final
	\end{block}
\end{frame}
...
\begin{frame}{Bosques Aleatorios y ``Gradient Tree Boosting''}
	Ensambles de pequeño clasificadores

	\begin{block}{Idea}
		Introducir aleatoriedad y disminuir la varianza
	\end{block}
...
Suponemos una distribución conocida para $p(\cdot)$
	...
	\begin{block}{función de decision }
...
\begin{frame}{Comparación de Fronteras de Decision}
		\includegraphics[width = 0.9 \paperwidth, height = 0.7 \paperheight,
...
Optimización de configuraciones de hiperparámetros $\mathcal{A}$ via estimación del $EPE$
...
Dado $(X,Y)$ y un clasificador $f_\theta \ t.q. \ \theta \in \Theta$
	\begin{enumerate}[I.]
		...
		\item Cross validar hiper parámetros de $f$ c/ $\trainset$
...
\begin{frame}{Inconvenientes}
		...
		\begin{block}{Sesgos en los datos}
			...
			\item Movilidad no implica prevalencia
...
% 		\begin{itemize}
% 			\item Comparación de resultados de movilidad con serologia por region
