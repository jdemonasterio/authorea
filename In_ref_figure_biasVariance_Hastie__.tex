...
Let $\mathcal {A}= \{A_1,A_{2},\dots \}}$ be a set family and $T$  another set, then $\mathcal {A}$ shatters $T$ if for every subset $t \in T$ there exists a set $A' \in \mathcal {A} $ such that $ T \cap A' = t$. The VC dimension of $\mathcal {A}$ is the biggest cardinality of a set shattered by $\mathcal {A}$. It is important to note that this means \texit{any} possible set shattered by $\mathcal {A}$
